...
export default function CarnetDigital() {
  ...
  // El QR codifica *exclusivamente* el user_id para que el comercio lo lea rápido
  const qrUrl = `https://api.qrserver.com/v1/create-qr-code/?size=250x250&data=${user.id}`;
        ...
        <div className="flex flex-col items-center mt-6 mb-8">
          <div className="relative group">
            ...
            <div className={`bg-white p-4 rounded-2xl shadow-xl relative ring-2 ${user.estado === 'APROBADO' ? 'ring-emerald-500/50 shadow-emerald-500/20' : 'ring-slate-200 shadow-slate-200'}`}>
              <img
                alt="QR Code"
                className={`w-40 h-40 ${user.estado !== 'APROBADO' ? 'opacity-30' : ''}`}
                src={qrUrl}
              />
              {user.estado !== 'APROBADO' && (
