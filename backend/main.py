...
if not service_role_key or service_role_key == key:
    logger.critical("🚨 ERROR CRÍTICO: No se detectó SUPABASE_SERVICE_KEY. Operaciones administrativas fallarán.")
else:
    # Loguear solo longitud y primeros caracteres para seguridad
    logger.info(f"✅ SUPABASE_SERVICE_KEY cargada (Length: {len(service_role_key)})")

try:
    # Aseguramos que el cliente admin se cree con la service_role_key explícitamente
...
async def change_password(data: PasswordChange, request: Request, user: TokenData = Depends(get_current_user)):
    ...
    1. Valida clave actual mediante un login temporal
    2. Actualiza clave usando Auth Admin API (Service Role)
    """
    logger.info(f"🔐 Intento de cambio de clave: {user.username}")
    
    # 1. Validar contraseña actual
    # Creamos un cliente temporal para no afectar el estado global
    ...
    try:
        ...
        if not auth_res.user:
            raise Exception("Credenciales actuales inválidas")
    except Exception as e:
        logger.warning(f"❌ Fallo validación de clave actual para {user.username}: {str(e)}")
        raise HTTPException(401, "La contraseña actual es incorrecta")

    # 2. Actualizar contraseña
    # Intentamos primero con Admin API ya que tenemos el UID y es más persistente
    try:
        # IMPORTANTE: Asegurarse de que 'supabase' sea el cliente con SERVICE_KEY
        if not supabase:
            raise Exception("Cliente Admin no disponible")
            
        # Intentar actualización administrativa
        supabase.auth.admin.update_user_by_id(
            user.uid,
            attributes={"password": data.new_password}
        )
        
        # Marcar cambio de clave en el perfil
        ...
        logger.error(f"🚨 Error crítico en cambio de clave (Admin API): {e}")
        # Fallback: Intentar con el cliente temporal autenticado recién creado
        try:
            temp_client.auth.update_user({"password": data.new_password})
            supabase.table("profiles").update({"temp_password": None}).eq("id", user.uid).execute()
            logger.info(f"✅ Contraseña actualizada vía fallback (User Context) para {user.username}")
            return {"message": "¡Contraseña actualizada con éxito!"}
        except Exception as fallback_err:
            logger.error(f"🚨 Falló fallback de cambio de clave: {fallback_err}")
            error_msg = str(e)
            if "User not allowed" in error_msg:
                error_msg = "Error de permisos en el servidor (Admin Key). Por favor contacte a soporte."
            raise HTTPException(400, f"No se pudo establecer la nueva contraseña: {error_msg}")
