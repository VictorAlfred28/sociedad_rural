...
# Inicializar cliente Supabase (Service Role bypass RLS para Admin functions like Auth signup backend side)
...
def get_formatted_private_key() -> str:
    ...
    # En caso de que venga todo junto sin saltos, validamos si tiene el formato estricto
    # Pero usualmente el replace("\\n", "\n") es lo único que hace falta si la variable se copió bien.
    if "-----BEGIN PRIVATE KEY-----" in formatted_key and "\n" not in formatted_key[28:]:
        # Es una string sin saltos de linea y sin los \n literales
        key_body = formatted_key.replace("-----BEGIN PRIVATE KEY-----", "").replace("-----END PRIVATE KEY-----", "").replace(" ", "")
