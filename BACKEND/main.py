...
@limiter.limit("10/minute")
def register_comercio(comercio: ComercioDTO, request: Request, background_tasks: BackgroundTasks):
    try:
        ...
        if "user already registered" in err_msg or "already exists" in err_msg and "email" in err_msg:
            ...
            friendly_detail = "El CUIT ingresado ya se encuentra registrado en el sistema."
        elif "duplicate key value" in err_msg:
            friendly_detail = "Algunos de los datos brindados ya se encuentran registrados."
        else:
            friendly_detail = f"Error al procesar el registro del comercio."
...
def get_municipios():
    ...
    try:
        ...
        res = supabase.table("localidades").select("id, nombre").eq("active", True).order("nombre").execute()
...
def generar_qr(request: Request):
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail=f"Error generando token QR: {str(e)}")
...
def validar_qr_dinamico(data: QRTokenValidarRequest):
    ...
    try:
        # 1. Buscamos el token
        result = supabase.table("qr_tokens").select("*").eq("token", data.token).execute()
        if not result.data:
            raise HTTPException(status_code=404, detail="El código QR es inválido o no existe.")
            
        qr_data = result.data[0]
        
        # 2. Verificamos si fue usado
        if qr_data.get("used"):
            raise HTTPException(status_code=400, detail="Este código QR ya fue utilizado. Pida al socio que genere uno nuevo.")
            
        # 3. Verificamos expiración
        expires_at_str = qr_data.get("expires_at")
        expires_at_dt = datetime.fromisoformat(expires_at_str)
        if datetime.now(pytz.utc) > expires_at_dt:
            raise HTTPException(status_code=400, detail="Este código QR ha expirado. Pida al socio que genere uno nuevo.")
            
        # 4. Marcamos como usado
        supabase.table("qr_tokens").update({"used": True}).eq("id", qr_data["id"]).execute()
            
        # 5. Pasamos a validar al socio
        socio_id = qr_data["user_id"]
        
        res = supabase.table("profiles").select("id, nombre_apellido, dni, rol, estado, municipio, titular_id, tipo_vinculo, perfiles_titulares:profiles!titular_id(nombre_apellido, estado)").eq("id", socio_id).execute()
...
async def upload_chat_image(
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail=f"Error en servidor al procesar imagen: {str(e)}")
...
async def chat_with_assistant(
    ...
    try:
        ...
        supabase.table("chat_history").insert({
            ...
            "metadata": {"mode": data.mode, "has_image": data.image_url is not None}
        }).execute()
        ...
        raise HTTPException(status_code=500, detail="Error procesando la solicitud de chat.")
...
def valida_socio(socio_id: str):
    if ENABLE_DYNAMIC_QR:
        raise HTTPException(status_code=400, detail="Los códigos QR estáticos ya no están activos por seguridad. Por favor solicite al socio generar un nuevo QR Dinámico desde su app.")
    """
...
def create_commerce(comercio: ComercioDTO, request: Request, background_tasks: BackgroundTasks, auth_user = Depends(get_current_admin_or_camara)):
    try:
...
def get_mis_pagos(current_user = Depends(get_current_user)):
    try:
        pagos = supabase.table("pagos_cuotas").select("*").eq("socio_id", current_user.id).order("fecha_vencimiento", desc=True).execute()
        return {"pagos": pagos.data}
    except Exception as e:
        raise HTTPException(status_code=500, detail=str(e))

@app.post("/api/pagos/subir-comprobante")
async def subir_comprobante(mes: int = Form(...), anio: int = Form(...), file: UploadFile = File(...), current_user = Depends(get_current_user)):
    try:
...
def get_pagos_pendientes(current_admin = Depends(get_current_admin)):
    try:
        ...
        res = supabase.table("pagos_cuotas").select("*, profiles!inner(nombre_apellido, dni)").eq("estado_pago", "PENDIENTE_VALIDACION").order("fecha_envio_comprobante").execute()
        return {"pendientes": res.data}
    except Exception as e:
        raise HTTPException(status_code=500, detail=str(e))

@app.post("/api/admin/pagos/aprobar")
def validar_pago(req: PagoActionRequest, background_tasks: BackgroundTasks, current_admin = Depends(get_current_admin)):
    pago_id = req.pago_id
    try:
        # Obtener datos del pago
        pago_res = supabase.table("pagos_cuotas").select("*, profiles!inner(nombre_apellido, dni, telefono)").eq("id", pago_id).execute()
        if not pago_res.data:
            raise HTTPException(status_code=404, detail="Pago no encontrado")
        ...
        supabase.table("profiles").update({"estado": "APROBADO", "motivo": None}).eq("id", pago["socio_id"]).execute()
        ...
        c.drawString(50, 510, f"Firma digital SRNC: Aprobado por tesorería.")
        ...
        if socio_profile.get("telefono"):
            ...
            background_tasks.add_task(enviar_whatsapp, socio_profile["telefono"], mensaje_wa)
